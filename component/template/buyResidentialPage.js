...
export default function BuyResidentialPage({PublishedAdv}){
    ...
    function filteringByCategory(){
        if(filter.category == 'all' || filter.category == '') {
            ...
            else if(!filter.rent && filter.sale) setFilterAdv(filteredAdv.filter(item => item.rent == 0 && item.mortgage == 0))
            ...
            else if(!filter.rent && filter.sale) setFilterAdv(filterCategory.filter(item => item.rent == 0 && item.mortgage == 0))
